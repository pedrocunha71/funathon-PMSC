...
In this exercise, you will generate predictions from the best RF model and compute all four evaluation metrics on the test set.
...
2.  Compute RMSE, MAE and R² using scikit-learn's metrics module.

::: {.callout-tip collapse="true" title="Click to view a hint"}

Import `root_mean_squared_error`, `mean_absolute_error` and `r2_score` from `sklearn.metrics`. Build a dictionary of results and print each value with a descriptive label. You can see the documentation of `sklearn.metrics` [here](https://scikit-learn.org/stable/api/sklearn.metrics.html).
...
for name, val in metrics_rf.items():
    print(f"  {name}: {val:.4f}")
```
...
1.  Write a `residuals_distribution(residuals, rmse)` function to plot the histogram of the residuals, with the parameters `residuals` (a pandas Series) and `rmse` (a float). 
...
def residuals_distribution(residuals: pd.Series, rmse: float):
    fig = plt.figure()
    plt.hist(residuals, bins=100, edgecolor="none")
    plt.axvline(0, color="red", linestyle="--", label="Residual = 0")
    plt.xlabel("Residual")
    plt.ylabel("Fréquency")
    plt.title(f"Residuals distribution (RMSE = {rmse:.3f})")
    plt.legend()
    return fig
```

2.  Plot the residuals distribution. Look at whether the histogram is centered around zero and whether it is approximately symmetric.
...
fig_residuals = residuals_distribution(residuals, metrics_rf["RMSE"])
plt.show()
```

3.  Write a `QQplot(y_test, y_pred)` function which computes 1000 quantile points for both series and plots them against each other. The parameters are `y_test` and `y_pred`, pandas series of observed and predicted target. The function should return the plot. 
...
def QQplot(y_test: pd.Series, y_pred: pd.Series):
    """
    ...
    fig = plt.figure()
    plt.scatter(q_real, q_predict, alpha=0.5, s=5, label="Quantiles")
    plt.plot(
        [q_real[0], q_real[-1]],
        [q_real[0], q_real[-1]],
        "r--", linewidth=1.5, label="Actual = Predicted"
    )
    plt.xlabel("Actual quantiles")
    plt.ylabel("Predicted quantiles")
    plt.title("QQ-plot: actual vs predicted")
    plt.legend()
    return fig

```

4.  Plot the QQ plot comparing the quantiles of the actual and predicted distributions. Check whether the points align closely along the diagonal, especially in the tails.
...
fig_pred = target_distribution(y_pred)
plt.title("Target distribution — predicted values")
plt.show()
```
...
```{python}
def plot_combined_distribution(y_test: pd.Series, y_pred: pd.Series):
    """
    ...
    axe_actual, y_sorted_actual = np.linspace(0, 100, len(np.sort(y_test))), np.sort(y_test)
    axe_pred, y_sorted_pred = np.linspace(0, 100, len(np.sort(y_pred))), np.sort(y_pred)

    # Create the plot
    fig, ax = plt.subplots()
    ax.plot(axe_actual, y_sorted_actual, label='Actual Values', color='blue')
    ax.plot(axe_pred, y_sorted_pred, label='Predicted Values', color='red')
